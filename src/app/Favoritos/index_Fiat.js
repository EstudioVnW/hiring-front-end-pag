...
const items = [
  { id: 1, name: 'Fiat Toro', image: "/imagens/FiatToro.png" },
];
    ...
    <div>
      <ul className='idUL'>
        {items.map(item => (
          <li key={item.id}>
           <h1>{item.name}</h1>
           <img src={item.image} alt={item.name} width="400" height="200" />
            <button className='buttonremov' onClick={() => toggleFavorite(item)}>
